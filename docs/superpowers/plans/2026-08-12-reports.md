...
- [ ] **Step 2: Apply the migration** — attempted, no local Supabase/Docker available in this environment; migration written and typechecked but unapplied/unverified against real Postgres (see roadmap.md)
...
- [x] **Step 3: Hand-add the TypeScript types**
...
- [x] **Step 2: Verify it compiles and runs its argument-parsing path**
...
- [x] **Step 1: Write the failing test (pure geometry function only — no component rendering test, matching this codebase's no-component-tests precedent)**
...
- [x] **Step 3: i18n — `reports` namespace**
...
- [x] **Step 4: Verify the JSON is valid and the app compiles**
...
- [x] **Step 3: Run it and manually verify the full pipeline end to end** — `REPORTS_SMTP_*`/`REPORTS_FROM_*` were added to `.env.local`; `pnpm test-fake-report` ran successfully against the real hosted Supabase project (migration 0039 is applied there, contrary to this file's earlier note — see roadmap.md). Printed 6 report rows (5 weekly + 1 monthly) with no thrown error, matching the expected output exactly; two preview emails sent to shengul@shengulai.com. Re-ran a second time and got back the identical 6 UUIDs, confirming the upsert-on-conflict semantics — no duplicate rows on a re-run. Inbox rendering (from-address, BCC, report-page charts, weekly-recap links) not independently confirmed from this session — that part needs Shengul's own inbox/browser check.
