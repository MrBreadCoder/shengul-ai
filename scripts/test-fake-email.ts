...
// supabase/migrations/0035_email_styles_table.sql, so this script exercises the
// exact wording a real client would get — not an approximation of it.
const STYLE_VOICE_INSTRUCTIONS: Record<'concise' | 'formal', string> = {
  ...
  formal:
    ...
    'naturally instead of listing capabilities generically. ' +
    '4. Personalize: use the strongest available dossier fact(s) to show this is not a mass-blast — ' +
    'prefer a (pain_point) or (news) fact over a bare (company) firmographic line. State it plainly; ' +
    'never add a claim about why it matters or what is "a priority" for them. Never fall back to a ' +
    'generic line like "I came across your company" or "I wanted to introduce ourselves". ' +
    '5. Ask: a qualifying question asking whether the recipient is the right person, followed by an ' +
    'offer to send the company profile, references, and product capabilities if so. Only mention the ' +
    'booking link here if it is clearly the natural next step. ' +
